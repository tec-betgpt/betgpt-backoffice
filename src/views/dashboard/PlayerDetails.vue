<template>
  <div class="space-y-6 p-10 max-[450px]:p-2 pb-16 w-full">
      ...
      <Card class="flex-1">
        ...
        <CardContent class="space-y-4">
          ...
          <div
            ...
            @scroll="handleScroll"
            class="relative h-[calc(100vh-24rem)] overflow-y-auto"
          >
            <div
              v-if="filteredHistory.length > 0"
              class="absolute left-1/2 transform -translate-x-1/2 h-full border-l border-gray-300"
            ></div>
            <div
            ...
            >
              <div
                ...
                :class="[
                  isSameTypeAsPrevious(index)
                    ? getEventTypeSide(event.type) === 'left'
                      ? 'justify-start'
                      : 'justify-end'
                    : getEventTypeSide(event.type) === 'left'
                      ? 'justify-start'
                ...
                >
                  ...
                  >
                    <CardContent class="p-3">
                      ...
                      <p class="text-xs mt-1 text-current opacity-75">
            ...
            <div
              v-if="filteredHistory.length === 0 && !isHistoryLoading"
            ...
            <div
              ...
              class="text-center p-4 text-sm text-muted-foreground"
            >
              Carregando mais...
            </div>
          </div>
    ...
    <div
      v-else-if="isLoading"
      class="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4"
    >
...
const getEventTypeSide = (type) => {
  if (!eventTypeSideMap.value[type]) {
    eventTypeSideMap.value[type] =
      availableSides[sideIndex % availableSides.length];
    sideIndex++;
  }
