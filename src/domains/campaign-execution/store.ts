...
import campaignExecutionApi from "./api";
import type {
...
export const useCampaignExecutionStore = defineStore("campaignExecution", {
  ...
  actions: {
    async prepare(campaignId: number) {
      ...
      try {
        const result = await campaignExecutionApi.prepare(campaignId);
    ...
    async runAction(
      ...
      try {
        const result = await campaignExecutionApi[action](campaignId);
        this.campaignId = campaignId;
        await this.fetchRun(campaignId);
        return result;
      } catch (error) {
    ...
    async fetchRun(campaignId: number) {
      ...
      try {
        const { run, waves } = await campaignExecutionApi.getRun(campaignId);
        this.campaignId = campaignId;
        this.run = run;
        this.waves = waves;
      } catch (error) {
    ...
    async fetchRunBatches(campaignId: number) {
      ...
      try {
        const { run, waves } = await campaignExecutionApi.getRunBatches(campaignId);
        this.campaignId = campaignId;
        this.run = run;
        this.waves = waves;
      } catch (error) {
    ...
    async fetchRunRecipients(
      ...
      try {
        const response = await campaignExecutionApi.getRunRecipients(
          campaignId,
