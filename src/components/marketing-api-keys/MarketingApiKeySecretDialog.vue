...
const props = defineProps<{
  secret: EphemeralMarketingApiSecret | null;
}>();
...
const title = computed(() =>
  props.secret?.kind === "rotated"
    ? t("marketing_api_keys.secret_title_rotated")
    : t("marketing_api_keys.secret_title_created"),
);
...
const rotationPolicyMessage = computed(() => {
  ...
  if (props.secret.rotationPolicy === "immediate") {
    return t("marketing_api_keys.secret_policy_immediate");
  }

  if (props.secret.rotationPolicy === "overlap") {
    return t("marketing_api_keys.secret_policy_overlap", {
...
async function copySecret() {
  ...
  try {
    ...
    toast(t("marketing_api_keys.secret_copied"));
  } catch {
...
function downloadSecret() {
  ...
  toast(t("marketing_api_keys.secret_downloaded"));
}
</script>
    ...
    >
      <DialogHeader>
        ...
        <DialogDescription class="flex items-start gap-2">
          ...
          <span>{{ t("marketing_api_keys.secret_warning") }}</span>
        </DialogDescription>
      </DialogHeader>
